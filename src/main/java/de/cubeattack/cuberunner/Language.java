...
public class Language {
   ...
   private HashMap<Language.Messages, String> messages;
   ...
   public Language(CubeRunner plugin, String fileName) {
      ...
      if (this.langageName != null) {
         ...
         Language.Messages[] var7;
         int var6 = (var7 = Language.Messages.values()).length;

         for(int var5 = 0; var5 < var6; ++var5) {
            Language.Messages message = var7[var5];
            this.messages.put(message, languageFile.get().getString(message.key, message.defaultMessage));
         }
   ...
   public String get(Language.Messages message) {
      return this.messages.get(message);
   }

   public void sendMsg(Player player, String message) {
      if (CubeRunner.get().getConfiguration().prefixInFrontOfEveryMessages) {
         player.sendMessage(ChatColor.translateAlternateColorCodes('&', this.get(Language.Messages.PREFIX_SHORT) + " " + message));
      } else {
   ...
   public void sendMsg(User user, String message) {
      this.sendMsg(user.getPlayer(), message);
   }

   public static Language getKeyLanguage(String fileName) {
      Iterator var2 = languages.entrySet().iterator();

      while(var2.hasNext()) {
         Entry<String, Language> local = (Entry)var2.next();
         if (((String)local.getKey()).equalsIgnoreCase(fileName)) {
            return (Language)local.getValue();
         }
      }
   ...
   public static Language getLanguage(String languageName) {
      ...
      while(var2.hasNext()) {
         Entry<String, Language> local = (Entry)var2.next();
         if (((Language)local.getValue()).langageName.equalsIgnoreCase(languageName)) {
            return (Language)local.getValue();
         }
      }
   ...
   public static Language getDefault() {
      Iterator var1 = languages.entrySet().iterator();

      while(var1.hasNext()) {
         Entry<String, Language> local = (Entry)var1.next();
         if (((String)local.getKey()).equalsIgnoreCase(CubeRunner.get().getConfiguration().language)) {
            return (Language)local.getValue();
         }
      }
