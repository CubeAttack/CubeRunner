...
public class CRSignQuit extends CRSign {
   public CRSignQuit(SignChangeEvent event) {
      super(event.getBlock().getLocation(), CRSign.SignType.QUIT);
      ...
      event.setLine(2, ChatColor.translateAlternateColorCodes('&', local.get(Language.Messages.KEYWORD_SIGN_QUIT)));
      event.setLine(3, "");
      signs.add(this);
   }

   public CRSignQuit(UUID uuid, Location location) {
      super(uuid, location, CRSign.SignType.QUIT);
   ...
   protected boolean updateSign(Language local, Sign sign) {
      ...
      sign.setLine(2, ChatColor.translateAlternateColorCodes('&', local.get(Language.Messages.KEYWORD_SIGN_QUIT)));
      sign.update();
      return true;
   }
