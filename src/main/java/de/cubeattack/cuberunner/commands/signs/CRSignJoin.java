...
public class CRSignJoin extends CRSignPlayers {
   ...
   public CRSignJoin(SignChangeEvent event, Arena arena) {
      super(event.getBlock().getLocation(), CRSign.SignType.JOIN);
      this.arena = arena;
      ...
      event.setLine(1, ChatColor.translateAlternateColorCodes('&', local.get(Language.Messages.KEYWORD_SIGN_JOIN)));
   ...
   public CRSignJoin(UUID uuid, Location location) {
      super(uuid, location, CRSign.SignType.JOIN);
   ...
   protected boolean updateSign(Language local, Sign sign) {
      ...
      sign.setLine(1, ChatColor.translateAlternateColorCodes('&', local.get(Language.Messages.KEYWORD_SIGN_JOIN)));
      sign.update();
      if (this.arena == null) {
         return false;
      } else {
   ...
   public void onInteract(Player player) {
      if (Permissions.hasPermission(CRCommand.JOIN.getPermission(), player, true)) {
         CRCommand.JOIN.execute(plugin, player, new String[]{"join", this.arena.getName()}, true);
      }
